...
A simple FastAPI-based service for maintaining release metadata with JSON logging, basic auth, and Prometheus metrics.
...
- `DATABASE_URL` (default: `sqlite:///./releasestore.db`; set to Postgres URL like `postgresql+psycopg2://user:pass@host/db`)
...
export DATABASE_URL="sqlite:///./releasestore.db"
python -m main
```
...
All API endpoints except `/`, `/readyz`, and `/livez` require authentication. In Swagger `/docs`, click "Authorize" and supply the configured username/password. Supplying no username will be rejected because the server defaults to `admin` when unset.

## Endpoints

- `GET /` → redirects to docs
- `GET /livez` (Lifecycle API) → liveness check
- `GET /readyz` (Lifecycle API) → readiness check (fails 500 if DB not reachable/ready)
...
```
This starts the API on `localhost:8000` and PostgreSQL on `localhost:5432` with `DATABASE_URL` pre-wired to the app.
